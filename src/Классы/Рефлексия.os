...
Процедура ИнициализироватьТипы() Экспорт
	
	ТаблицаИзвестныхТипов = Рефлектор.ИзвестныеТипы();
	
	Соответствие = Новый Соответствие();
	
	Для Каждого Элемент Из ТаблицаИзвестныхТипов Цикл
		Структура = Новый Структура("Значение, Примитивный, Пользовательский, Коллекция");
		ЗаполнитьЗначенияСвойств(Структура, Элемент);
		Соответствие.Вставить(Элемент.Имя, Структура);
	КонецЦикла;
	
	ИзвестныеТипы = Новый ФиксированноеСоответствие(Соответствие);
	
КонецПроцедуры

Функция ТаблицаИзвестныхТипов(Фильтр = Неопределено) Экспорт
	Возврат Рефлектор.ИзвестныеТипы(Фильтр);
КонецФункции

Функция ИзвестныеТипы() Экспорт
	Возврат ИзвестныеТипы;
КонецФункции
...
Функция АннотацииПараметровМетода(Объект, ИмяМетода) Экспорт
	...
	Если НайденныеСтроки.Количество() Тогда
		...
		Для Каждого ДанныеПараметра Из НайденныеСтроки[0].Параметры Цикл
			
			СвойстваПараметра = Новый Структура("Имя, ПоЗначению, ЕстьЗначениеПоУмолчанию, ЗначениеПоУмолчанию, Аннотации");
			ЗаполнитьЗначенияСвойств(СвойстваПараметра, ДанныеПараметра);
			СвойстваПараметра.Аннотации = ТаблицаАннотацийВСтруктуру(ДанныеПараметра.Аннотации);
			СтруктураПараметров.Вставить(ДанныеПараметра.Имя, СвойстваПараметра);
...
Функция ТипСуществуетВКонтексте(ИмяТипа) Экспорт
	
	НайденныеСтроки = Рефлектор.ИзвестныеТипы(Новый Структура("Имя", ИмяТипа));
