	...
	Для Каждого ДанныеОбъекта Из Объекты Цикл
		...
		Для Каждого Свойство Из Свойства Цикл
			...
			Если Свойство.Имя = "Этот" Тогда
				...
				ИначеЕсли АннотацииСвойства.Свойство("Наследует") Тогда
...
Процедура СобратьИЗарегистрироватьОбъекты() Экспорт
	...
	Для Каждого Элемент Из Объекты Цикл
		...
		Если НЕ ОписаниеОбъекта.ЭтоПеречисление() Тогда
			Продолжить;
		КонецЕсли;
		
		ЗначенияСвойствПеречисления = ОписаниеОбъекта.ЗначенияСвойствПеречисления();
		ОбъектПеречисления = ОписаниеОбъекта.ЭталонныйОбъект();
		
		Для Каждого Переменная Из ЗначенияСвойствПеречисления Цикл
			Рефлексия.УстановитьСвойство(ОбъектПеречисления, Переменная.Ключ, Переменная.Значение);
		КонецЦикла;
		
		ОбъектыОдиночки.Вставить(ОписаниеОбъекта.Имя(), ОбъектПеречисления);
...
Функция ОписаниеОбъекта(ИмяТипа) Экспорт
	...
	Иначе
		Возврат ?(НайденныеДанные.ЭтоПеречисление(), НайденныеДанные.ЭталонныйОбъект(), НайденныеДанные);
	КонецЕсли;
