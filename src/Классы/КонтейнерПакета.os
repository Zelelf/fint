...
Процедура ПриСозданииОбъекта(КонтекстПриложения, ОписанияСкриптов, ПутьВКонтексте)
	...
	ОбъектыОдиночки = Новый Соответствие();
	...
	Для Каждого ДанныеОбъекта Из Объекты Цикл
		...
		Для Каждого Свойство Из Свойства Цикл
			...
			Если Свойство.Имя = "Этот" Тогда
				...
				Если АннотацииСвойства.Свойство("Общий") Тогда
					ЭтоОбщий = Истина;
					Рефлексия.УстановитьСвойство(ОписаниеОбъекта, "ЭтоОбщий", Истина);
				КонецЕсли;
				...
				ИначеЕсли АннотацииСвойства.Свойство("Интерфейс") Тогда
					ЭтоИнтерфейс = Истина;
					Рефлексия.УстановитьСвойство(ОписаниеОбъекта, "ЭтоИнтерфейс", Истина);
				ИначеЕсли АннотацииСвойства.Свойство("Перечисление") Тогда
					
					ЭтоПеречисление = Истина;
					Рефлексия.УстановитьСвойство(ОписаниеОбъекта, "ЭтоПеречисление", Истина);
					
					Если ТипЗнч(АннотацииСвойства.Перечисление) = Тип("Строка") Тогда
						
						ИзвестныеТипы = Рефлексия.ИзвестныеТипы();
						ДанныеТипаПеречисления = ИзвестныеТипы[АннотацииСвойства.Перечисление];
						
						Если ДанныеТипаПеречисления = Неопределено Тогда
							ВызватьИсключение СтрШаблон("Тип перечисления ""%1"" не найден. Объект: %2", АннотацииСвойства.Перечисление, ПутьКОбъекту);
						КонецЕсли;
						
						Если Тип(АннотацииСвойства.Перечисление) <> Тип("Строка") И Тип(АннотацииСвойства.Перечисление) <> Тип("Число") Тогда
							ВызватьИсключение СтрШаблон("Тип перечисления должен быть Строка или Число вы указали ""%1"". Объект: %2",
								АннотацииСвойства.Перечисление, ПутьКОбъекту);
						КонецЕсли;
						
						Если Методы.Количество() > 1 Тогда
							ВызватьИсключение СтрШаблон("Перечисления не должны содержать в себе процедуры или функции. Объект: %1", ПутьКОбъекту);
						КонецЕсли;
						
						ТипПеречисления = Тип(АннотацииСвойства.Перечисление);
			...
			Если ЭтоПеречисление Тогда
				...
				Если НЕ Свойство.Экспорт Тогда
					ВызватьИсключение СтрШаблон("Свойство перечисления ""%1"" не экспортное. Объект: %2",
						Свойство.Имя, ПутьКОбъекту);
				КонецЕсли;
				
				Если НЕ АннотацииСвойства.Свойство("Значение") Тогда
					ВызватьИсключение СтрШаблон("Свойство перечисления %1 не имеет аннотации ""Значение"". Объект: %2",
						Свойство.Имя, ПутьКОбъекту);
				КонецЕсли;
				
				Если ТипЗнч(АннотацииСвойства.Значение) <> ТипПеречисления Тогда
					ВызватьИсключение СтрШаблон("Значение перечисления %1 не соответствует требуемому типу, ожидался тип ""%3"" бы передан ""%4"". Объект: %2",
						Свойство.Имя, ПутьКОбъекту, Строка(ТипПеречисления), Строка(ТипЗнч(АннотацииСвойства.Значение)));
				КонецЕсли;
				
				ЗначенияСвойствПеречисления.Вставить(Свойство.Имя, АннотацииСвойства.Значение);
		...
		Если ЭтоПеречисление Тогда
			Рефлексия.УстановитьСвойство(ОписаниеОбъекта, "ЗначенияСвойствПеречисления", ЗначенияСвойствПеречисления);
			Продолжить;
		КонецЕсли;
		...
		Если СтрокиМетодаКонструктора.Количество() Тогда
			...
			Если ЭтоОбщий И Метод.Параметры.Количество() Тогда
				ВызватьИсключение СтрШаблон("Метод конструктора не должен содержать параметров для общего (одиночного) объекта. Объект: %1", ПутьКОбъекту);
			КонецЕсли;
...
Процедура СобратьИЗарегистрироватьОбъекты() Экспорт
	...
	Для Каждого Элемент Из Объекты Цикл
		...
		ОписаниеОбъекта = Элемент.Значение;
		
		Если НЕ ОписаниеОбъекта.ЭтоПеречисление() И НЕ ОписаниеОбъекта.ЭтоОбщий() Тогда
			Продолжить;
		КонецЕсли;

		Если ОписаниеОбъекта.ЭтоПеречисление() Тогда

			ЗначенияСвойствПеречисления = ОписаниеОбъекта.ЗначенияСвойствПеречисления();
			ОбъектОдиночка = ОписаниеОбъекта.ЭталонныйОбъект();
			
			Для Каждого Переменная Из ЗначенияСвойствПеречисления Цикл
				Рефлексия.УстановитьСвойство(ОбъектОдиночка, Переменная.Ключ, Переменная.Значение);
			КонецЦикла;

		КонецЕсли;
		
		Если ОписаниеОбъекта.ЭтоОбщий() Тогда
			ОбъектОдиночка = ОписаниеОбъекта.Создать();
		КонецЕсли;

		ОбъектыОдиночки.Вставить(ОписаниеОбъекта.Имя(), ОбъектОдиночка);
...
Функция СоздатьОбъект(Знач ИмяТипа, Знач ВыполнятьКонструктор = Истина, Парам1 = NULL, Парам2 = NULL, Парам3 = NULL,
	...
	Для Каждого ДанныеСвойства Из Рефлексия.ПолучитьТаблицуСвойств(ЭталонныйОбъект) Цикл
		...
		ЗначениеСвойства = Рефлексия.ПолучитьСвойство(ЭталонныйОбъект, ДанныеСвойства.Имя);

		Если ТипЗнч(ЗначениеСвойства) = Тип("ОписаниеОбъекта") Тогда
			
			Если ЗначениеСвойства.ЭтоОбщий() ИЛИ ЗначениеСвойства.ЭтоПеречисление() Тогда
				ЗначениеСвойства = ЗначениеСвойства.Контейнер().ОбъектОдиночка(ЗначениеСвойства.Имя());
			КонецЕсли;

		КонецЕсли;

		Рефлексия.УстановитьСвойство(Объект, ДанныеСвойства.Имя, ЗначениеСвойства);
		Рефлексия.УстановитьСвойство(ЭкспортныйОбъект, ДанныеСвойства.Имя, ЗначениеСвойства);
		Рефлексия.УстановитьСвойство(ЛокальныйОбъект, ДанныеСвойства.Имя, ЗначениеСвойства);
...
Функция ОбъектОдиночка(ИмяТипа) Экспорт
	
	НайденныеДанные = ОбъектыОдиночки[ИмяТипа];
	
	Если НайденныеДанные = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Одиночный объект %2 не найден в пространстве имен %1", Путь, ИмяТипа);
	Иначе
		Возврат НайденныеДанные;
	КонецЕсли;
