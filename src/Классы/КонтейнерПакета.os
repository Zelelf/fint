	...
	Для Каждого ДанныеОбъекта Из Объекты Цикл
		...
		Для Каждого Свойство Из Свойства Цикл
			...
			Если ПоследняяАннотацияИмпорта <> Неопределено И НЕ АннотацииСвойства.Количество() Тогда
				...
				Иначе
					ОписаниеОбъектаИмпорта = Контекст.ОписаниеОбъекта(Свойство.Имя, ПоследняяАннотацияИмпорта);
				КонецЕсли;
...
Процедура ДобавитьПерехватчикиКОбъекту(Знач Объект, Декоратор, ЛокальныйДекоратор, Метод, ГенерируемыйМетод = Ложь)
	...
	Если Метод.ЭтоФункция Тогда
		Перехватчики.Добавить(ПерехватчикПроверкиТипаВозвращаемогоЗначения(Объект, Метод));
	КонецЕсли;
...
Функция СоздатьОбъект(Знач ИмяТипа, Знач ВыполнятьКонструктор = Истина, Парам1 = NULL, Парам2 = NULL, Парам3 = NULL,
	...
	Если ОписаниеБазовогоКласса <> Неопределено Тогда
		...
		БазовыйОбъект = СоздатьОбъект(ОписаниеБазовогоКласса.Имя(), Ложь);
...
Функция ПерехватчикПроверкиТиповПараметровМетода(Объект, ДанныеМетода, ГенерируемыйМетод = Ложь)
	...
	ТекстМетода = "ДанныеПараметров = Новый Структура();";
	...
	Для Каждого Параметр Из АннотацииПараметровМетода Цикл
		...
		ШаблонСтроки = ШаблонСтроки + Символы.ПС + "ДанныеПараметров.Вставить(""%1"", Новый Структура(""ДанныеТипов, Значение"", ДанныеТипов, %1));";
		
		ТекстМетода = ТекстМетода + Символы.ПС + СтрШаблон(ШаблонСтроки, Параметр.Ключ); // BSLLS:IncorrectUseOfStrTemplate-off
		
	КонецЦикла;
	
	ТекстМетода = ТекстМетода + Символы.ПС + "ФИНТ_Контейнер.ПроверитьТипыПараметров(ЭтотОбъект, """ + ДанныеМетода.Имя + """, ДанныеПараметров);";
...
Функция ПерехватчикПроверкиТипаВозвращаемогоЗначения(Объект, ДанныеМетода)
	...
	Для Каждого КлючЗначение Из АннотацииМетода Цикл
		ИмяТипа = КлючЗначение.Ключ;
	КонецЦикла;
	
	ТекстМетода = "ФИНТ_Контейнер.ПроверитьТипВозвращающегоЗначения(ЭтотОбъект, """ + ДанныеМетода.Имя + """, """ + ИмяТипа + """, Декоратор_ВозвращаемоеИзМетодаЗначение);";
...
Процедура ПроверитьТипыПараметров(Объект, ИмяМетода, ДанныеПараметров) Экспорт
	...
	Для Каждого Переменная Из ДанныеПараметров Цикл
		...
		Для Каждого ДанныеОжидаемогоТипа Из ОжидаемыеТипы Цикл
			...
			ИменаОжидаемыхТипов = ?(ИменаОжидаемыхТипов = "", "", ", ") + Строка(ДанныеОжидаемогоТипа.ОсновнойТип);
			
			Если ДанныеОжидаемогоТипа.Типы.Найти(ТипЗначение) <> Неопределено Тогда
		...
		Если НЕ ЕстьСовпадениеТипа Тогда
			ШаблонТекстаИсключения = "Несоответствие типов параметра ""%1"" метода ""%2"" ожидался(лись) тип(ы) ""%3"" передан ""%4""";
			ВызватьИсключение СтрШаблон(ШаблонТекстаИсключения, Переменная.Ключ, ИмяМетода, ИменаОжидаемыхТипов, Строка(ТипЗначение));
		КонецЕсли;
...
Процедура ПроверитьТипВозвращающегоЗначения(Объект, ИмяМетода, ИмяОжидаемогоТипа, ВозвращаемоеЗначение) Экспорт
	
	Если ИмяОжидаемогоТипа = "Любой" Тогда
		Возврат;
	КонецЕсли;
	
	ИсходныйОбъект = Рефлексия.ПолучитьСвойство(Объект, "Декоратор_ИсходныйОбъект");
	ОписаниеОжидаемогоТипа = Неопределено;
	
	Если Рефлексия.СвойствоСуществует(Объект, ИмяОжидаемогоТипа) Тогда
		ОписаниеОжидаемогоТипа = Рефлексия.ПолучитьСвойство(Объект, ИмяОжидаемогоТипа);
	Иначе
		Рефлексия.ТипСуществуетВКонтексте(ИмяОжидаемогоТипа);
	КонецЕсли;
	
	ОжидаемыйТип = ?(ТипЗнч(ОписаниеОжидаемогоТипа) <> Тип("ОписаниеОбъекта"), Тип(ИмяОжидаемогоТипа), ОписаниеОжидаемогоТипа.ТипОбъекта());
	ТипЗначение = ТипЗнч(ВозвращаемоеЗначение);
	ПроверочныйТип = ТипЗначение;
	
	ДанныеТипа = Рефлексия.ИзвестныеТипы()[Строка(ОжидаемыйТип)];
	
	Если ДанныеТипа.Пользовательский И ТипЗначение = Тип(ОписаниеОжидаемогоТипа.ИмяТипаЭкспортногоОбъекта()) Тогда
		ПроверочныйТип = ТипЗнч(Рефлексия.ПолучитьСвойство(ВозвращаемоеЗначение, "Декоратор_ИсходныйОбъект"));
	КонецЕсли;
	
	Если ПроверочныйТип <> ОжидаемыйТип Тогда
		ВызватьИсключение СтрШаблон("Функция %1.%2 возвращает значение не верного типа, ожидался тип ""%3"" возвращен ""%4""",
			Строка(ТипЗнч(ИсходныйОбъект)), ИмяМетода, ИмяОжидаемогоТипа, Строка(ПроверочныйТип));
	КонецЕсли;
